...
class GigaChatLLM(LLM):
    """Кастомный LLM для работы с GigaChat через LangChain."""
...
prompt_template = PromptTemplate(
    ...
    template=(
        "Сгенерируй мотивационную цитату на тему: {category}."
    )
)
...
def generate_motivation(category: str) -> str:
    """Генерация мотивационной цитаты с использованием LangChain и GigaChat."""
    try:
        result = llm_chain.run(category=category)
        return result
    except Exception as e:
        logging.error(f"Ошибка при генерации мотивации через LangChain: {e}")
        return "Не удалось получить мотивационную фразу. Попробуйте позже."
...
async def cmd_menu2(message: Message):
    ...
    builder.button(text='📓 Просмотреть все записи', callback_data='all')
    ...
    builder.button(text="☄️ Посмотреть цель", callback_data='notion')
...
'''
# Функция для отправки цитат с периодичностью (через asyncio.sleep())
...
async def show_goals_for_reminders(call: CallbackQuery, state: FSMContext):
    user_id = call.from_user.id
    ...
    builder.add(KeyboardButton(text="🔙 Вернуться в меню"))
    keyboard = builder.as_markup(resize_keyboard=True)
    await call.message.answer("Выберите цель для просмотра информации:", reply_markup=keyboard)
...
async def process_goal_selection(message: types.Message, state: FSMContext):
    goal_name = message.text
    user_id = message.from_user.id
    goals = load_goals_from_file(user_id)
    goal = next((g for g in goals if g["name"] == goal_name), None)
    if not goal:
        await message.answer("Цель не найдена. Выберите другую цель или нажмите '🔙 Вернуться в меню'.")
        return

    # Формируем текст с информацией о цели
    goal_info = (
        f"📋 Информация о цели:\n\n"
        f"🔸 Название: {goal.get('name', 'Без названия')}\n"
        f"🔸 Описание: {goal.get('description', 'Описание отсутствует')}\n"
        f"🔸 Дата начала: {goal.get('start_date', 'Не указана')}\n"
        f"🔸 Дата окончания: {goal.get('end_date', 'Не указана')}\n"
        f"🔸 Статус: {goal.get('status', 'Неизвестен')}"
    )
    builder = ReplyKeyboardBuilder()
    builder.add(KeyboardButton(text="🔙 Вернуться в меню"))
    keyboard = builder.as_markup(resize_keyboard=True)

    await message.answer(goal_info, reply_markup=keyboard)
    await state.clear()
# КОНЕЦ ЦЕЛИ
